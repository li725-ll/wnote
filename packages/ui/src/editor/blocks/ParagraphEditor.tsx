...
export function ParagraphEditor({ block }: ParagraphEditorProps) {
  const { state, dispatch } = useEditor();
  const ref = useRef<HTMLDivElement>(null);
  const node = block.node as Extract<BlockNode, { type: "paragraph" }>;
  ...
  const handleBlur = useCallback(() => {
    const el = ref.current;
    if (!el) return;
    const raw = domToMarkdown(el).replace(/\n$/, "");
    const parsed = parseBlock(raw) ?? { type: "paragraph" as const, children: [], raw };
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent<HTMLDivElement>) => {
      ...
      if (e.key === "Backspace") {
        ...
        if (sel && sel.isCollapsed && isAtStart(el, sel)) {
          ...
          const raw = domToMarkdown(el).replace(/\n$/, "");
          const merged = prev.node.raw + raw;
    ...
    [block.id, state.blocks, dispatch],
  );
